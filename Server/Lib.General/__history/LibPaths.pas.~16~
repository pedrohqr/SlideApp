...
begin
  ...
  if not FileExists(Result) then
    raise Exception.Create('SERVER: Arquivo "fbclient.dll" não encontrado em: ' + Result);
end;
...
function Path_DB : String;
begin
  Result := 'C:\Users\pedro\Desktop\projeto_slide\Server\DataBase' + PathDelim + 'SERVER.FDB';
  if not FileExists(Result) then
...
begin
  ...
  if not FileExists(Result) then
    raise Exception.Create('SERVER: Arquivo "config.ini" não encontrado em: ' + Result);
